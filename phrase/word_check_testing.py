...
def candidate_from_ngrams(ngram_words,word,n):
    ...
    for ngram in ngrams_list:
        ...
        if ngram not in ngram_words:
            continue
        for length in range(max(len(word)-LEN_PRUNE_DOWN,2),len(word)+LEN_PRUNE_UP+1):
...
def get_dict_unigrams():
    d = {}
    with open('../ngrams/unixdict.txt') as f:
        for line in f.read().splitlines():
            word = line.split('\t')[0]
            unigrams = set(ngrams(word,1))
            d[word] = unigrams
    return d


def jaccard_prune(misspelt_word,candidates,dict_bigrams,dict_unigrams):
    #print len(candidates)
    if len(misspelt_word) <= 6:
        ngrams_misspelt_word = set(ngrams(misspelt_word,1))
    else:
    ...
    for word in candidates:
        ...
        if len(misspelt_word) <=6  and word[0] != misspelt_word[0]:
            continue

        if len(misspelt_word) <= 6:
            ngrams_word = dict_unigrams[word]
        else:
...
def run_input(prior_frequencies,ngram_words,matrices,dict_bigrams,dict_unigrams,soundex_index):
    while True:
        ...
        candidate_selections = candidate_from_ngrams(ngram_words,misspelt_word,NGRAM_N)
        candidate_selections = jaccard_prune(misspelt_word,candidate_selections,dict_bigrams,dict_unigrams)
        a = candidates_from_soundex(misspelt_word,soundex_index)
        
        #print candidate_selections
        
        candidate_selections = list(set(candidate_selections) | set(a))
        
        #print len(candidate_selections)
        # read dictionary file into a trie
        trie = TrieNode()
        if 'extacy' in candidate_selections:
        	print "hi"
...
soundex_index = create_soundex_index()
dict_unigrams = get_dict_unigrams()
run_input(prior_frequencies,ngram_words,matrices,dict_bigrams,dict_unigrams,soundex_index)
